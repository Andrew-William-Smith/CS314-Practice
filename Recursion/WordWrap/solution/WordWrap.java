/* CS 314 PRACTICE PROBLEM --- WORD WRAP (SOLUTION)
...
package Recursion.WordWrap.solution;
...
public class WordWrap {
    ...
    public static int wrap(Scanner input, int lineLength) {
        // Start the line length at the limit so that any words will result in a new line
        return wordWrapHelp(input, lineLength, lineLength);
    }

    /**
     * Helper method for <code>WordWrap.wrap()</code>.  Consumes a single word from the paragraph and determines if the
     * paragraph should wrap at that point.
     *
     * @param input Contains the paragraph to wrap.
     * @param maxLength The maximum number of characters in each line.
     * @param currentLength The length of the line currently under construction.
     * @return The number of lines necessary to wrap the sub-paragraph beginning at the current word.
     */
    private static int wordWrapHelp(Scanner input, int maxLength, int currentLength) {
        // Base case: no more words in the paragraph!
        if (!input.hasNext()) {
            return 0;
        }

        int lineIncrement = 0;
        int curWordLen = input.next().length();
        if (currentLength + curWordLen >= maxLength) {
            // This word must go on the next line as it exceeds the right margin
            lineIncrement = 1;
            currentLength = curWordLen + 1;
        } else {
            // This word goes on the current line
            currentLength += curWordLen + 1;
        }

        // Determine the length of the following sub-paragraph
        return lineIncrement + wordWrapHelp(input, maxLength, currentLength);
    }
